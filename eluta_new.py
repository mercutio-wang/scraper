...
def eluta_collection_module(path_to_bin):
    ...
    file_name = path_to_bin + "/el-result_" + today + ".csv"
    ...
    with open(file_name, 'ab') as csvfile:
        ...
        for province in provinces:
            ...
            for item in full_list:
                ...
                result['html_file'] = encode_cleanup(get_html(result,today))
                if result is not None:
                    writer.writerow(result)
                    
def get_html(entry, today):
    url = entry['url']
    try:
        ...
        return ''
    fname = 'html/'+entry['native_id']+'_'+today+'.txt'
