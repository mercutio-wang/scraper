...
def write_csv(result_dict, cntry = 'ca'):
    fieldnames = ['Job title',
                  ...
                  'Posted',
                  'Reference code']
    fname = 'monster_result_'+cntry+'_'+str(datetime.date.today())+'.csv'
...
def monster_indi_result(pmeta):
    ...
    try:
        sourecode = urllib.urlopen(pmeta['link']).read()
        sourcecode = re.sub('<br\s*?>', ' ', sourcecode)
        soup = BeautifulSoup.BeautifulSoup(sourecode)
    except:
